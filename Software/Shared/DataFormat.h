...
that indicates the START of the measurement. Remember to call
Validate() to set the validity of the data.
所有派生类中timeStamp一项均指明测量 开始 时的时间戳. 注意测量完成后
调用Validate()正确设置数据有效性.
*/
struct DataBase {
...
#pragma pack()
	...
	_position_d(const _position&);
	_position_d& operator=(const _position&);
} Position_d;
...
#pragma pack()
	...
	_position(const _position_d&);
	_position& operator=(const _position_d&);
} Position;
_position_d::_position_d(const _position &pp) : x(pp.x), y(pp.y), z(pp.z) {}
_position_d& _position_d::operator=(const _position &pp) {
	x = pp.x; y = pp.y; z = pp.z; return (*this);
}
_position::_position(const _position_d &pp) : x((float)pp.x), y((float)pp.y), z((float)pp.z) {}
_position& _position::operator=(const _position_d &pp) {
	x = (float)pp.x; y = (float)pp.y; z = (float)pp.z; return (*this);
}
...
#pragma pack()
	...
	_orie(const _orie_d&);
	_orie& operator=(const _orie_d&);
} Orientation;
_orie_d::_orie_d(const _orie &oo) : yaw(oo.yaw), pitch(oo.pitch), roll(oo.roll) {}
_orie_d& _orie_d::operator=(const _orie &oo) {
	yaw = oo.yaw; pitch = oo.pitch; roll = oo.roll; return (*this);
}
_orie::_orie(const _orie_d &oo) : yaw((float)oo.yaw), pitch((float)oo.pitch), roll((float)oo.roll) {}
_orie& _orie::operator=(const _orie_d &oo) {
	yaw = (float)oo.yaw; pitch = (float)oo.pitch; roll = (float)oo.roll; return (*this);
}

// 6D位姿
struct _pose_d_;
struct _pose_;
typedef struct _pose_d_ {
	Position_d pos;
	Orientation_d orie;
	_pose_d_() {}
	_pose_d_(const _pose_&);
	_pose_d_& operator=(const _pose_&);
} Pose_d;
typedef struct _pose_ {
	Position pos;
	Orientation orie;
	_pose_() {}
	_pose_(const _pose_d_&);
	_pose_& operator=(const _pose_d_&);
} Pose;
_pose_d_::_pose_d_(const _pose_& pp) : pos(pp.pos), orie() {}
_pose_d_& _pose_d_::operator=(const _pose_ &pp) {
	pos = pp.pos; orie = pp.orie; return (*this);
}
_pose_::_pose_(const _pose_d_ &pp) : pos(pp.pos), orie(pp.orie) {}
_pose_& _pose_::operator=(const _pose_d_ &pp) {
	pos = pp.pos; orie = pp.orie; return (*this);
}
